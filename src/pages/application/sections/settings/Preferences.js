...
const Preferences = () => {
    ...
    return (
        ...
        self-start max-w-[970px] w-full rounded-[10px] sm:rounded-[20px] md:rounded-[30px] lg:rounded-[40px] px-[20px] sm:px-[30px] md:px-[40px] lg:px-[50px] py-[20px] md:py-[30px] lg:py-[40px] flex flex-col sm:flex-row justify-between items-center`}>
            <form className="flex flex-col h-full w-full justify-center items-center" onSubmit={(e) => handleSubmit(e)}>
                <div className="w-full h-full flex flex-wrap gap-5 flex-col sm:flex-row justify-between items-start mb-[60px]">
                    <div className="max-w-[300px] w-full relative flex flex-col justify-center items-start">
                        ...
                        {langDropDown && (
                            <div className={`${localStorage.getItem("mode") === "Light Mode" ? 'bg-white' : 'bg-[#BCB8B8]'}  
                            flex flex-col absolute top-full mt-2 w-full border-[1px] border-[#CED4DA] rounded-xl shadow-lg overflow-hidden z-50`}>
                                <button
                    ...
                    <div className="max-w-[300px] w-full relative flex flex-col justify-center items-start">
                        ...
                        <button
                            ...
                            {selectedMode ? t(`settings.preferences.appearance.${selectedMode}`) : 'Light Mode'}
                            <BiChevronDown className="text-[15px] lg:text-[25px]" />
                        </button>
                        {modeDropDown && (
                            <div className={`${localStorage.getItem("mode") === "Light Mode" ? 'bg-white' : 'bg-[#BCB8B8]'}  
